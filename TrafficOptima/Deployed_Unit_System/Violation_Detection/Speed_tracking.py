...
if not os.path.exists(speed_record_folder_name):
    os.makedirs(speed_record_folder_name)
    os.makedirs(speed_record_folder_name+"//exceeded")

# write_recorded_speed_file
speed_record_text_file_location = speed_record_folder_name + "//speed_record.txt"
...
class Euclidean_distance_tracker:
    ...
    def capture(self, img, x, y, h, w, sp, id):
        if (self.capf[id] == 0):
            ...
            file = speed_record_folder_name + '//' + n + '.jpg'
            file_name = n + '.jpg'
            # cv2.imwrite(file, crop_img)
            self.count += 1
            filet = open(speed_record_text_file_location, "a")
            if (sp > limit):
                file2 = speed_record_folder_name + '//exceeded//' + n + '.jpg'
